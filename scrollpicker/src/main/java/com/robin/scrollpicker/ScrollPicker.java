 ...
 * 上下滚动控件
 */
public class ScrollPicker<T> extends View {
    /**
     * 数据源
     */
    ...
    public ScrollPicker(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public ScrollPicker(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int drawDataPos = drawnSelectedPos - mHalfVisibleItemCount - 1; drawDataPos <= drawnSelectedPos + mHalfVisibleItemCount + 1; drawDataPos++) {
            ...
            if (!TextUtils.isEmpty(mIndicatorText)) {
                drawText = drawText + mIndicatorText;
            }
            //在中间位置的Item作为被选中的。
